...
在本機執行 Python 腳本時，MYSQL_HOST=127.0.0.1。
...
☁️ BigQuery：輕量 ELT
本專案採用輕量 ELT：將 MySQL 的原始資料表同步至 BQ_DATASET_RAW，並在 BQ_DATASET_ANALYTICS 層建立視圖/物化表，供查詢與視覺化使用。
...
轉換：在 BigQuery 建視圖與物化表（供 Metabase / 查詢）
本機直跑
# 安裝 BQ 相關套件（若未安裝）
...
公式：(谷底 − 高峰) ÷ 高峰，其中 谷底發生在高峰之後；在整個期間內取最小值。
...
公式：平均( 近12個月現金股利 ÷ 當日價格 )，對期間 P 取平均。
...
公式：取 期末日 的 dividend_12m（若期末日為空，取最近一筆非空值）。
...
div_yield_12m_avg（近 12M 平均殖利率）


dividend_12m_latest（近 12M 配息最新值）


互動：


依 total_return 由高到低排序。


以 ticker、date range 篩選動態刷新 KPI。


後續若需要擴充，再在此章節下方逐一新增卡片條目即可。
...
git add .
git commit -m "docs+feat: 新增 RSI/MA/MACD/KD/BBANDS 與 Metabase 新版面板；BigQuery View 範例"
git push
